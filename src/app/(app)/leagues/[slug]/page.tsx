	...
	return (
		<div className="min-h-screen bg-gray-50 py-24 sm:py-32 dark:bg-gray-900">
			<div className="mx-auto max-w-2xl px-6 lg:max-w-7xl lg:px-8">
				...
				<div className={`mt-10 grid gap-4 sm:mt-16 lg:grid-cols-3 lg:grid-rows-${pricing.length > 1 ? pricing.length : 2}`}>
								...
								{`${cards?.[0]?.contents || '\nLeague Overview'}`
									...
									.map((line, index) => (
										<p
											className={
												index > 0
													? 'mt-2 max-w-lg text-sm/6 text-gray-600 max-lg:text-justify dark:text-gray-400'
													: 'mt-2 text-lg font-medium tracking-tight text-gray-950 dark:text-white'
											}
											key={index}
										>
											{line}
								...
								{`${cards?.[1]?.contents || '\nLeague Overview'}`
									...
									.map((line, index) => (
										<p
											className={
												index > 0
													? 'my-2 max-w-lg text-sm/6 text-gray-600 dark:text-gray-400'
													: 'my-4 text-sm/6 font-medium tracking-tight text-gray-950 dark:text-white'
											}
											key={index}
										>
											{line}
					...
					{pricing?.map((card, lindex) => (
						<div
							key={lindex}
							className={`relative max-lg:row-start-${lindex + 2} lg:col-start-2 lg:row-start-${lindex + 1} ${pricing.length < 2 ? 'row-span-2' : 'row-span-1'}`}
						>
									...
									<div className="mt-2 flex justify-between text-lg font-medium tracking-tight text-gray-950 max-lg:text-center dark:text-white">
										...
										{card.description
											? `${card.description}`.split('\n').map((line, lidx) => {
													return <p key={lidx} className="mt-2 max-w-lg text-sm/6 text-gray-600 dark:text-gray-400">{line.split(' ').map((word, ii) => {
					...
					<div
						className={`relative ${pricing?.length ? `max-lg:row-start-${pricing?.length + 2}` : ''} lg:col-start-3 lg:row-start-1`}
					>
							...
							<div className="px-8 pt-8 pb-3 sm:px-10 sm:pt-10 sm:pb-0">
								{`${cards?.[2]?.contents || '\nLeague Overview'}`
									...
									.map((line, lidx) => (
										<p
											className={
												lidx > 0 && cards?.[2]?.content_type === 'title and paragraph'
													? 'my-2 max-w-lg text-sm/6 text-gray-600 dark:text-gray-400'
													: 'mt-2 text-lg font-medium tracking-tight text-gray-950 max-lg:text-center dark:text-white'
											}
											key={lidx}
										>
											{line}
					...
					<div className={`relative lg:row-start-${pricing.length > 1 ? pricing.length : 2} lg:col-start-3`}>
